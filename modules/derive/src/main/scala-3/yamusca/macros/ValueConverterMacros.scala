...
object ValueConverterMacros:
  inline final def deriveConverter[A <: Product](using inline A: Mirror.ProductOf[A]) =
    deriveViaMirror[A]

  inline given deriveViaMirror[A <:Product](using m: Mirror.ProductOf[A]): ValueConverter[A] =
    ...
    new ValueConverter[A]:
      def apply(a: A): Value =
        ...
        Value.fromContext(Context.from { key =>
