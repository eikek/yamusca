...
object ValueConverterMacros {
  ...
  def valueConverterImpl[T: c.WeakTypeTag](c: Context): c.Tree = {
    ...
    val fields = tpa.decls.collect {
      case field if field.isMethod && field.asMethod.isCaseAccessor => field.name.toTermName
    }
    val cases = fields.map { f =>
      q"""if (${TermName(f.toString).decodedName.toString} == key) { result = Some(a.$f.asMustacheValue) }"""
    }

    if (fields.isEmpty) {
      c.abort(c.enclosingPosition, s"No accessors for type `$tpa'. This is only meant for case clases.")
    } else {
