...
class ParserCombSpec extends AnyFlatSpec with Matchers {
  ...
  it should "work without whitespace" in {
    standaloneOr(consume("x"))(ParseInput("x")).
      left.map(e => fail(e._2)).
      map(t => {
  ...
  "consumeUntilEndSection" should "consume input until end section" in {
    ...
    check("abcd{{#blup}}x{{/blup}}yz{{/hello}}blabla", "hello", "abcd{{#blup}}x{{/blup}}yz", "blabla")
  }
  ...
  "parseSection" should "parse sections" in {
    ...
    check("  {{#test}}\n hallo{{/test}}", Section("test", Seq(Literal(" hallo")), false), "")
  ...
  it should "recognize custom delimiters" in {
    parseTemplate(ParseInput("{{=<< >>=}}<<#test>><<name>><</test>><<={{ }}=>>{{help}}")).
      left.map(e => fail(e._2)).
      map(t => {
        t._2 should be (Template(Seq(
          Literal(""),
    ...
    parseTemplate(ParseInput("{{=<< >>=}}{{hello}}")).
      left.map(e => fail(e._2)).
      map(t => {
        t._2 should be (Template(Seq(
  ...
  it should "not remove whitespace on tags" in {
    parseTemplate(ParseInput(" {{#boolean}}YES{{/boolean}}\n {{#boolean}}GOOD{{/boolean}}\n")).
      left.map(e => fail(e._2)).
      map(t => {
        t._2 should be (Template(Seq(
          Literal(" "),
